...
def insert_usuario(user_data: UsuarioSchema):
    ...
    Valida que el nombre de usuario y el correo no estén duplicados.
    """
    data = user_data.dict()
    data.pop("id", None)
    ...
    # Recupera el id recién creado y asigna rol 'usuario' por defecto
...
def login(payload: LoginSchema):
    ...
    if not row:
        raise HTTPException(
            ...
            detail="El usuario ingresado no existe"   # ⬅️ antes: "Credenciales inválidas"
        )
    ...
    if not ok:
        raise HTTPException(
            status_code=HTTP_401_UNAUTHORIZED,
            detail="La contraseña es incorrecta"   # ⬅️ antes: "Credenciales inválidas"
        )
